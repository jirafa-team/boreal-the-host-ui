...
import { ChevronLeft, ChevronRight, CheckCircle2, Circle, Clock, MapPin } from "lucide-react";
...
export interface StaffTasksViewProps {
  tasks: UiTask[];
  onToggleTask: (id: number | string) => void;
}

export function mapStaffTaskToUi(t: StaffTask): UiTask {
...
export function StaffTasksView({ tasks, onToggleTask }: StaffTasksViewProps) {
  ...
  const inProgressCount = tasks.filter(
    (t) => t.status === "in-progress" && t.date === dateFilter
  ).length;
  const completedCount = tasks.filter(
    (t) => t.status === "completed" && t.date === dateFilter
  ).length;
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-semibold whitespace-nowrap flex-shrink-0 ${getStatusColor(task.status)}`}
                  >
                    {task.status === "pending"
                      ? "Pendiente"
                ...
                <p
                  className={`text-sm mb-2 ${task.status === "completed" ? "text-gray-400" : "text-gray-600"}`}
                >
                  {task.description}
                </p>
